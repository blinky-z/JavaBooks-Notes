...
Также не стоит создавать классы-исключения, которые не расширяют ни *Exception*, ни *RuntimeException*, ни *Error*. Обычно подразумевается что такое исключение - это checked exception. Но таких случаев, когда понадобится такой класс, просто нет, и если делать так, то это только запутает клиента API.